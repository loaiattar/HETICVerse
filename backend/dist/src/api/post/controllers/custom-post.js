...
exports.default = {
     ...
     */
    getPosts: async (ctx) => {
        const { limit = '20', page = '1', sort = 'newest', community, user: userId, search, type } = ctx.request.query;
        const { user } = ctx.state;
        try {
            ...
            if (community) {
                ...
                if (user) {
                    ...
                    const memberCommunityIds = memberships
                        .map(m => { var _a; return (_a = m.community) === null || _a === void 0 ? void 0 : _a.id; })
            ...
            const postsWithDetails = await Promise.all(posts.map(async (post) => {
                ...
                // Check if user has voted on this post
                let userVote = null;
                const vote = await strapi.entityService.findMany('api::vote.vote', {
                    filters: {
                        users_permissions_user: {
                            id: {
                                $eq: user.id
                ...
                return {
                    ...
                    userVote: userVote,
                    isBookmarked,
                    user: ((_a = post.attributes.user) === null || _a === void 0 ? void 0 : _a.data) ? {
                    ...
                    community: ((_b = post.attributes.community) === null || _b === void 0 ? void 0 : _b.data) ? {
                    ...
                    image: ((_c = post.attributes.image) === null || _c === void 0 ? void 0 : _c.data) ? {
     ...
     */
    getFeed: async (ctx) => {
        const { limit = '20', page = '1', sort = 'newest' } = ctx.request.query;
        const { user } = ctx.state;
        if (!user) {
    ...
    getTrending: async (ctx) => {
        const { limit = '20', page = '1', community } = ctx.request.query;
        const { user } = ctx.state;
        try {
            const limitNum = parseInt(limit);
            const pageNum = parseInt(page);
