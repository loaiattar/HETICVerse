...
export default {
  ...
  getFollowers: async (ctx) => {
    ...
    const { limit = '20', page = '1' } = ctx.request.query as FollowQueryParams;
    const { user } = ctx.state;
  ...
  suggestUsers: async (ctx) => {
    const { limit = '10' } = ctx.request.query as FollowQueryParams;
    const { user } = ctx.state;
    ...
    try {
      ...
      const userFollowerCounts = await Promise.all(
