...
exports.default = {
    ...
    createVideoCall: async (ctx) => {
        const { title, description, startTime, maxParticipants, isPrivate, communityId } = ctx.request.body;
    ...
    updateVideoCall: async (ctx) => {
        const { id } = ctx.params;
        const { title, description, status, startTime, endTime, maxParticipants, isPrivate } = ctx.request.body;
        const { user } = ctx.state;
        if (!user) {
    ...
    getVideoCall: async (ctx) => {
        ...
        try {
            ...
            return {
                ...
                accessCode: ((_a = videoCall.creator) === null || _a === void 0 ? void 0 : _a.id) === (user === null || user === void 0 ? void 0 : user.id) ? videoCall.accessCode : undefined,
    ...
    getVideoCalls: async (ctx) => {
        const { limit = '20', page = '1', status = 'active', community, user: userId } = ctx.request.query;
        const { user } = ctx.state;
        try {
            ...
            // Format response
            const formattedVideoCalls = await Promise.all(videoCalls.map(async (videoCall) => {
    ...
    joinVideoCall: async (ctx) => {
        ...
        try {
            ...
            if (videoCall.isPrivate && videoCall.accessCode !== accessCode && ((_a = videoCall.creator) === null || _a === void 0 ? void 0 : _a.id) !== user.id) {
