...
/**
 * Custom video call participant controller
 */
...
export default {
  ...
  getVideoCallParticipants: async (ctx) => {
    ...
    const { status = 'joined', limit = '50', page = '1' } = ctx.request.query as ParticipantQueryParams;
  ...
  getUserParticipations: async (ctx) => {
    const { limit = '20', page = '1', status } = ctx.request.query as ParticipantQueryParams;
    const { user } = ctx.state;
