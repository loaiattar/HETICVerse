...
exports.default = {
    ...
    initialize({ strapi }) {
        io = new socket_io_1.Server(strapi.server.httpServer, {
            cors: {
    ...
    async notifyOfflineRoomMembers(roomId, senderId, message) {
        try {
            const room = await strapi.db.query('api::chat-room.chat-room').findOne({
                where: { id: roomId },
                populate: '*'
            ...
            const offlineParticipants = room.participants.filter(participant => !onlineUserIds.includes(participant.id) && participant.id !== senderId);
