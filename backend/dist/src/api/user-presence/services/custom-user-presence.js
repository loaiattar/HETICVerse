...
exports.default = {
    ...
    updateLastActive: async (userId) => {
        try {
            ...
            if (presence) {
                return await strapi.entityService.update('api::user-presence.user-presence', presence.id, {
                    data: {
                        lastActive: new Date()
                    }
                });
    ...
    cleanupOldRecords: async (daysThreshold = 30) => {
        try {
            ...
            for (const record of oldRecords) {
                await strapi.entityService.delete('api::user-presence.user-presence', record.id);
            }
