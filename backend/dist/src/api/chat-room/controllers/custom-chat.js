...
exports.default = {
    ...
    createDirectChat: async (ctx) => {
        ...
        try {
            ...
            const recipient = await strapi.entityService.findOne('plugin::users-permissions.user', recipientId, { populate: '*' });
            if (!recipient) {
    ...
    addParticipant: async (ctx) => {
        ...
        try {
            ...
            const userToAdd = await strapi.entityService.findOne('plugin::users-permissions.user', userId, { populate: '*' });
            if (!userToAdd) {
