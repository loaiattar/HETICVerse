...
module.exports = {
  ...
  getMyNotifications: async (ctx) => {
    const { 
      ...
      type, 
      read 
    } = ctx.request.query as NotificationQueryParams;
  ...
  markAllAsRead: async (ctx) => {
    const { type } = ctx.request.query as NotificationQueryParams;
    const { user } = ctx.state;
  ...
  updatePreferences: async (ctx) => {
    ...
    try {
      ...
      if (communityNotifications !== undefined) updateData.communityNotifications = communityNotifications;
      if (mentionNotifications !== undefined) updateData.mentionNotifications = mentionNotifications;
