...
export default {
  ...
  getTrending: async (ctx) => {
    const { 
      ...
      category,
      community
    } = ctx.request.query as TrendingQueryParams;
    ...
    try {
      ...
      const postBookmarks: Record<string, number> = {};
      bookmarkCounts.forEach((row: any) => {
      ...
      const postComments: Record<string, number> = {};
      ...
      const scoredPosts = posts.map(post => {
        ...
        const trendingScore = (
          commentCount * 3 +  // Each comment is worth 3 points
